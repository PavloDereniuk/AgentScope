...
## Стан: Epics 1-8 CLOSED — DAY 6 (2026-04-16)

### Поточна задача
**Epic 9** — Polish + Deploy + Submission.
...
- ✅ **Епік 8 (Demo + Landing): 10/10 CLOSED** — 3 demo scripts (trader/yield/nft) + trigger-anomaly + setup-wallets + Astro 4 landing page (Hero/Features/DemoVideo/CTA). Lint+build зелені.
- 📦 Епік 9: deploy + відео + submission
...
**Epic 9: Polish + Deploy + Submission** (решта задач — 9.1-9.10):
- **9.1-9.3** Railway deploy: api + ingestion + cron
- **9.4-9.5** Vercel deploy: dashboard + landing
- **9.6** Supabase RLS final check
- **9.7** Mainnet migration test (опційно)
- **9.8** Demo script записати
- **9.9** YouTube відео
- **9.10** README + submission
